...
router.get('/getOrders',FetchUser,  async(req, res) =>{
    try {
        // const orders = await Orders.find({buyer: req.user.id}).populate('product').populate('products').populate('buyer', 'firstName').lean();
        // const productIds = orders.flatMap((order) => order.product);
